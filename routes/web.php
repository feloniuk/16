...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::middleware('role:admin')->group(function () {
        ...
        Route::prefix('telegram')->name('telegram.')->group(function () {
            ...
            Route::post('/clear-webhook', [TelegramBotController::class, 'clearWebhook'])->name('clear-webhook');
        });
        
        // Журнал операций (полный доступ)
        Route::resource('inventory-logs', InventoryLogController::class)->only(['index', 'show']);
    ...
    // === МАРШРУТЫ ДЛЯ АДМИНИСТРАТОРОВ И ЗАВЕДУЮЩИХ СКЛАДОМ ===
    Route::middleware('role:admin,warehouse_manager')->group(function () {
        
        // Инвентарь (полное управление)
