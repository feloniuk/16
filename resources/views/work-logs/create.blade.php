...
@section('title', 'Додати запис про роботу')
                ...
                <form method="POST" action="{{ route('work-logs.store') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="work_type" id="work_type" class="form-select @error('work_type') is-invalid @enderror" required>
                            ...
                            <option value="inventory_transfer" {{ old('work_type') == 'inventory_transfer' ? 'selected' : '' }}>Переміщення інвентарю</option>
                            <option value="cartridge_replacement" {{ old('work_type') == 'cartridge_replacement' ? 'selected' : '' }}>Заміна картриджа</option>
                            <option value="repair_sent" {{ old('work_type') == 'repair_sent' ? 'selected' : '' }}>Відправка на ремонт</option>
                            <option value="repair_returned" {{ old('work_type') == 'repair_returned' ? 'selected' : '' }}>Повернення з ремонту</option>
                            <option value="manual" {{ old('work_type') == 'manual' ? 'selected' : '' }}>Інше</option>
                        </select>
                        @error('work_type')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror"
                                  rows="4" placeholder="Детально опишіть проведену роботу..." maxlength="1000" required>{{ old('description') }}</textarea>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" name="room_number" id="room_number" class="form-control @error('room_number') is-invalid @enderror"
                                       placeholder="Наприклад: 101, каб. 5" maxlength="50" value="{{ old('room_number') }}" required>
                                @error('room_number')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="performed_at" id="performed_at" class="form-control @error('performed_at') is-invalid @enderror"
                               value="{{ old('performed_at', today()->toDateString()) }}" required>
                        @error('performed_at')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror"
                                  rows="3" placeholder="Додаткова інформація..." maxlength="2000">{{ old('notes') }}</textarea>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check"></i> Створити запис
                        </button>
                    </div>
                </form>
            </div>
