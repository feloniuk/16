...
@section('title', 'Редагувати заявку на ремонт')
...
<div class="row justify-content-center">
    <div class="col-lg-11">
        <div class="stats-card p-4">
            <div class="mb-4">
                <h4>Редагувати заявку на ремонт</h4>
                <p class="text-muted">Заявка {{ $repairOrder->order_number }}</p>
            </div>

            <form method="POST" action="{{ route('repair-orders.update', $repairOrder) }}" id="repairForm">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="description" id="description"
                               ...
                               value="{{ old('description', $repairOrder->description) }}" placeholder="Короткий опис заявки">
                        @error('description')
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="repair_master_id" id="repair_master_id"
                            ...
                            @foreach($masters as $master)
                                <option value="{{ $master->id }}" {{ old('repair_master_id', $repairOrder->repair_master_id) == $master->id ? 'selected' : '' }}>
                                    {{ $master->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror"
                              rows="3" placeholder="Додаткова інформація про ремонт">{{ old('notes', $repairOrder->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-between">
                    <a href="{{ route('repair-orders.show', $repairOrder) }}" class="btn btn-outline-secondary">
                        <i class="bi bi-arrow-left"></i> Назад
                    </a>
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save"></i> Оновити заявку
                    </button>
                </div>
            </form>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    // Загрузить существующие элементы
    @foreach($repairOrder->items as $item)
        addItemRow({
            id: {{ $item->equipment_id }},
            equipment_type: '{{ addslashes($item->equipment->equipment_type) }}',
            inventory_number: '{{ $item->equipment->inventory_number }}',
            repair_description: '{{ addslashes($item->repair_description) }}',
            cost: {{ $item->cost ?? 0 }}
        });
    @endforeach

    // Если элементов нет, добавить пустую строку
